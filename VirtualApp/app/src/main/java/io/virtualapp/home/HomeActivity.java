...
public class HomeActivity extends VActivity implements HomeContract.HomeView {
            ...
            if (!data.isLoading()) {
                ...
                /*try {
                    if (data instanceof PackageAppData){
                        Log.e("zz","PackageAppData");
                    }else if (data instanceof MultiplePackageAppData){
                        Log.e("zz",((MultiplePackageAppData) data).userId+"");
                    }
                }catch (Exception e){
    ...
    private void onAddAppButtonClick() {
       ...
       /* try {
            ...
            AppInfoLite appList = new AppInfoLite(packageInfo.packageName,packageInfo.applicationInfo.publicSourceDir,true);
            mPresenter.addApp(appList);
    ...
    public void loadFinish(List<AppData> list) {
        //判断名字
        /*List<AppData> PgCheck_list = new ArrayList<AppData>();
        for (int i = 0;i < list.size();i++){
            AppData appData = list.get(i);
            if (appData.getName().equals("微信")){
                PgCheck_list.add(appData);
            }
        }
        PgCheck_list.add(new AddAppButton(this));
        */
